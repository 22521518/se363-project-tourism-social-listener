...
try:
    ...
    from projects.services.processing.tasks.intention.models import Base
    from projects.services.processing.tasks.intention.dto import (
        IntentionDTO, IntentionType, SentimentType
    )
    ...
    try:
        ...
        for intention in intentions:
            data.append({
                ...
                'sentiment': intention.sentiment.value,
                'processed_at': intention.processed_at,
                'model_used': intention.model_used
            })
...
def show_update_form(intention_row):
    ...
    with st.form(key=f"update_form_{intention_row['id']}"):
        ...
        with col2:
            ...
            current_sentiment_idx = sentiment_types.index(intention_row['sentiment'])
            
            new_sentiment = st.selectbox(
                "Sentiment",
                options=sentiment_types,
                index=current_sentiment_idx,
                key=f"sentiment_{intention_row['id']}"
            )
        ...
        if submit:
            handle_update_intention(
                ...
                model_used=intention_row['model_used']
            )
...
def handle_update_intention(intention_id, source_id, raw_text, new_intention, new_sentiment, model_used):
    ...
    try:
        ...
        updated_dto = IntentionDTO(
            ...
            sentiment=SentimentType(new_sentiment),
...
def render_statistics_overview(stats):
    ...
    with col3:
        most_common_sentiment = max(
            stats.by_sentiment.items(),
            key=lambda x: x[1],
            default=("N/A", 0),
        )
        st.metric(
            "Top Sentiment",
            most_common_sentiment[0].title(),
            f"{most_common_sentiment[1]:,}"
        )
    ...
    with col2:
        ...
        for k, v in sorted(stats.by_sentiment.items(), key=lambda x: -x[1]):
            st.write(f"**{k.title()}**: {v:,}")
...
def render_intention_distribution(stats):
    ...
    fig = px.pie(
        values=list(stats.by_intention_type.values()),
        names=list(stats.by_intention_type.keys()),
...
def render_sentiment_distribution(stats):
    """Render sentiment distribution chart."""
    if not stats.by_sentiment:
    ...
    sentiments = list(stats.by_sentiment.keys())
    values = list(stats.by_sentiment.values())
    colors = [color_map.get(s, '#6c757d') for s in sentiments]
...
def render_intentions_table(df):
    ...
    event = st.dataframe(
        display_df[['raw_text', 'intention_type', 'sentiment', 'processed_at', 'model_used']],
        column_config={
...
def render_dashboard():
    ...
    with col2:
        render_sentiment_distribution(stats)
