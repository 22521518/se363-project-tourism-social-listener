"""
YouTube NER Batch Processor for Location Extraction.

Fetches YouTube comments that have been ingested but not yet processed by
location extraction, and processes them using NER model (Davlan/xlm-roberta-base-ner-hrl).

Usage:
    python -m projects.services.processing.tasks.location_extraction.batch.youtube_ner_batch_processor
...
try:
    ...
    from ..extractors.ner_extractor import NERLocationExtractor
...
class YouTubeNERBatchProcessor:
    """
    Batch processor for YouTube comments location extraction using NER.
    
    Uses Davlan/xlm-roberta-base-ner-hrl model for multilingual NER.
    """
    ...
    def __init__(
        ...
        # Initialize NER extractor and DAO
        ...
        # Ensure location_extractions table exists
        self._ensure_tables_exist()
        
        logger.info(f"YouTubeNERBatchProcessor initialized - batch_size={batch_size}, max_records={max_records}")
    ...
    def process_comment(self, comment: Dict[str, Any]) -> Optional[PersistenceLocationDTO]:
        """Process a single comment for location extraction using NER."""
        ...
        try:
            ...
            from ..dto.location_result import Location
            locations = [
                Location(
                    word=loc["word"],
                    score=loc["score"],
    ...
    def run(self) -> Dict[str, Any]:
        ...
        logger.info("Starting YouTube NER batch location extraction...")
        ...
        logger.info(f"NER Batch processing complete: {stats}")
        return stats
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process YouTube comments for location extraction using NER"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    processor = YouTubeNERBatchProcessor(
